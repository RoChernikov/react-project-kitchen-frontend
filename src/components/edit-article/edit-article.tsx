    ...
    <div className={styles.editArticle}>
      ...
      <form className={styles.editArticle__form}>
        <label className={styles.editArticle__label}>
          ...
          <input className={styles.editArticle__input} name='title' onChange={onTitleChange} value={title} required/>
        </label>
        <label className={styles.editArticle__label}>
          <p className={styles.editArticle__labelText}>О чем статья</p>
          <textarea onChange={onAboutChange} name='about' className={`${styles.editArticle__input} ${styles.editArticle__input_textarea}`} rows={4} value={about} required /> 
        </label>
        <label className={styles.editArticle__label}>
          <p className={styles.editArticle__labelText}>URL изображения (опционально)</p>
          <input className={styles.editArticle__input} onChange={onImageChange} name='image' value={image} /> 
        </label>
        <label className={styles.editArticle__label}>
          <p className={styles.editArticle__labelText}>Текст статьи</p>
          <textarea onChange={onTextChange} name='text' className={`${styles.editArticle__input} ${styles.editArticle__input_textarea}`} rows={24} value={text} required /> 
        </label>
        <label className={styles.editArticle__label}>
          <p className={styles.editArticle__labelText}>Теги (через запятую)</p>
          <input  className={styles.editArticle__input} onChange={onTagsChange} name='tags' value={tags} required /> 
        </label>
        <div className={styles.editArticle__buttons}>          
          <button className={styles.editArticle__submitBtn} type="submit">Сохранить запись</button>
